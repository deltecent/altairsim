...
// divisor (OUT BA+2). It does NOT model:
//
...
//     the constant with the real state machine.
...
public:
    ...
    // Non-owning; the UART owns the stream.
    ByteStream* unitStream(const std::string& unit) override {
        return unit == "line" ? &u_.stream() : nullptr;
    }
...
private:
    ...
    uint8_t out3_ = 0;  // OUT BA+3 -- 6860 modem control (inert)
};
