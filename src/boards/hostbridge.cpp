...
std::string HostBridgeBoard::sandboxRoot() const {
    ...
    // AN EMPTY hostdir IS NAMED, NOT RESOLVED -- and that is not laziness, it is the only
    // value this can have on a card nobody has configured. docs/manual/ref/ is GENERATED by
    // running the binary and printing each property of a default board, so an absolute path
    // here would bake the machine that built the manual into the manual ("...ard/build/"),
    // differ on every host, and leak the maintainer's directory layout to every reader.
    // The phrase is stable, and it is exactly as precise: SHOW PATHS prints that directory
    // on the line above.
    if (hostdir_.empty()) return "(the directory you launched from)";

    // ABSOLUTE otherwise. A relative answer would be the question again: `xfer` is what you
    // asked about, and `xfer` is not an answer. Only a LIVE machine ever gets here.
