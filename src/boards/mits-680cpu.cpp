...
void Cpu6800Board::serialize(StateWriter& w) const {
    Board::serialize(w);
    core_->serialize(w);
}

void Cpu6800Board::deserialize(StateReader& r) {
    Board::deserialize(r);
    core_->deserialize(r);
}
...
// is no shared base because two dozen-line cards do not need one, and the Z80 card
// makes the same copy (mits-z80cpu.cpp).
std::vector<Property> Cpu6800Board::properties() {
...
void Cpu6800Board::publishPolicy() {
    if (!clock_) return;
    clock_->setHz(clockHz_);
    clock_->setIdle(idle_);
}

std::vector<UnitDef> Cpu6800Board::units() const {
    return {{"6800", UnitKind::Cpu, "active"}};
}
