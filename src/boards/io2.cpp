...
namespace {
Io2Board::EndpointResolver g_resolver;
} // namespace

void Io2Board::setResolver(EndpointResolver r) { g_resolver = std::move(r); }
...
// carry DAV/TBMT, and whether the inverter gate is engaged. The IO-2's jumpers let it
// imitate each of these; the first, `sior0`, is the board's own MITS-SIO-Rev-0
// personality and its default.
//
...
const std::vector<Io2Builtin>& io2Builtins() {
    static const std::vector<Io2Builtin> kBuiltins = {
        // MITS SIO Rev 0 (AY-5-1013) -- the IO-2's own serial personality and the
        // DEFAULT. Status/data at BASE+0/BASE+1; DAV is status bit 0, TBMT is bit 7,
        // both routed through the inverter gate (asserted reads 0, i.e. active low).
        // This is exactly what the SSM 8080 System Monitor expects on its console
        // (roms/SSM-8080MON/SSM_8080MonV10.asm: spins while D0=1 waiting for a byte,
        // spins while D7=1 waiting to send).
        {"sior0", "MITS SIO Rev 0 (AY-5-1013): status/data at BASE+0/BASE+1, "
                  "DAV=bit0 TBMT=bit7, inverter gate on (active low). The SSM IO-2 "
                  "serial personality and the SSM 8080 monitor console",
         Io2Profile{/*status*/ 0x00, /*data*/ 0x01, /*dav*/ 0, /*tbmt*/ 7, /*inverterGate*/ true}},
        ...
        // is a separate write-only port -- and the IO-2 discards control writes -- so it
        // collapses cleanly to the status/data pair.
        {"tuart", "Cromemco TU-ART (TMS 5501): status/data at BASE+0/BASE+1, "
                  "DAV=bit6 TBMT=bit7, inverter gate off (active high)",
         Io2Profile{/*status*/ 0x00, /*data*/ 0x01, /*dav*/ 6, /*tbmt*/ 7, /*inverterGate*/ false}},
        ...
        {"imsai-sio2", "IMSAI SIO-2 ch A (Intel 8251): data/status at BASE+2/BASE+3, "
         ...
         Io2Profile{/*status*/ 0x03, /*data*/ 0x02, /*dav*/ 1, /*tbmt*/ 0, /*inverterGate*/ false}},
        ...
        {"compupro-if2", "CompuPro Interfacer II (1602/1863 UART): data/status at "
         ...
         Io2Profile{/*status*/ 0x01, /*data*/ 0x00, /*dav*/ 1, /*tbmt*/ 0, /*inverterGate*/ false}},

        // CompuPro System Support 1 (Signetics 2651 USART): the 2651 is a four-port chip
        // (data/status/mode/command); the IO-2 models only the data+status pair, so its
        // mode and command ports fall to unclaimed I/O -- harmless for a polled console,
        // since those are init-only writes. Status reads TxRDY in bit 0 and RxRDY in bit
        // 1, both active high -- the same logical shape as the Interfacer II. Default base
        ...
        {"compupro-ss1", "CompuPro System Support 1 (2651 USART): data/status at "
         ...
         Io2Profile{/*status*/ 0x5D, /*data*/ 0x5C, /*dav*/ 1, /*tbmt*/ 0, /*inverterGate*/ false}},
    };
    return kBuiltins;
}
...
Io2Board::Io2Board() {
    ...
    // Come up as `sior0` -- the SSM IO-2's own personality and the SSM 8080 monitor
    // console -- so a bare `type="io2"` is immediately useful. profile_ already reads
    // "sior0"; copy its straps into the live fields.
    for (const auto& b : io2Builtins())
        if (b.name == "sior0") { applyProfile(b.profile); break; }
}

// Two ports, and only two: the status/control port and the data port. Everything
// else on the bus is somebody else's.
bool Io2Board::decodes(const BusCycle& c) const {
...
// SYNTHESIZE THE STATUS BYTE. Start at 0, set the DAV bit from readable() and the
// TBMT bit from writable(), each XORed against the inverter gate.
//
// "true = asserted" is the stream's contract (host/stream.h); the inverter gate is a
// fact about THIS card's pins (both status bits share one inverting buffer), applied
// here and nowhere else. A bit reads 1 when its signal is asserted UNLESS the gate is
// engaged, in which case asserted reads 0.
uint8_t Io2Board::statusByte() const {
    uint8_t s = 0;
    if (stream_->readable() != straps_.inverterGate) s |= (uint8_t)(1u << straps_.davBit);
    if (stream_->writable() != straps_.inverterGate) s |= (uint8_t)(1u << straps_.tbmtBit);
    return s;
}

uint8_t Io2Board::read(const BusCycle& c) {
...
void Io2Board::write(const BusCycle& c) {
    if (c.port() == straps_.dataPort) {
...
void Io2Board::applyProfile(const Io2Profile& p) { straps_ = p; }
...
void Io2Board::configChanged() {
    decodeChanged();
    programLine();
}

std::vector<std::string> Io2Board::drainLog() {
...
void Io2Board::serialize(StateWriter& w) const {
    Board::serialize(w);  // enabled_
    w.u64(rxBytes_);
}

void Io2Board::deserialize(StateReader& r) {
    Board::deserialize(r);
    rxBytes_ = r.u64();
...
std::vector<Property> Io2Board::properties() {
    std::vector<Property> p;

    // THE PROFILE SELECTOR. Its choices are `custom` plus every built-in name, so
    // `SHOW io20` and tab-completion list what an operator can select. Choosing a
    ...
    // `profile=sior0` + an overridden `status_port` reload in the right order.
    {
        Property x;
        x.name    = "profile";
        ...
        x.kind    = Kind::Enum;
        x.choices = {"custom"};
        for (const auto& b : io2Builtins()) x.choices.push_back(b.name);
    ...
    // The two status-bit positions (0-7), named for the UART's signals.
    {
        ...
        x.min  = 0;
        x.max  = 7;
        x.get  = [this] { return Value::ofInt(straps_.davBit); };
        x.set  = [this](const Value& v, std::string&) {
            straps_.davBit = (uint8_t)v.i();
            return true;
        };
        p.push_back(std::move(x));
    }
    {
        ...
        x.min  = 0;
        x.max  = 7;
        x.get  = [this] { return Value::ofInt(straps_.tbmtBit); };
        x.set  = [this](const Value& v, std::string&) {
            straps_.tbmtBit = (uint8_t)v.i();
            return true;
        };
        p.push_back(std::move(x));
    }
    // The inverter gate -- ONE knob for both status bits. On the real IO-2 they share a
    // single inverting buffer, so there is no way to invert one without the other.
    {
        Property x;
        ...
        x.help = "Route both status bits through the inverter gate -- asserted DAV/TBMT "
                 "read 0 (active low)";
        x.kind = Kind::Bool;
        x.get  = [this] { return Value::ofBool(straps_.inverterGate); };
        x.set  = [this](const Value& v, std::string&) {
            straps_.inverterGate = v.b();
            return true;
        };
        p.push_back(std::move(x));
    }
...
std::vector<UnitDef> Io2Board::units() const {
    return {{"serial", UnitKind::Serial, stream_->describe()}};
}

std::vector<MapEntry> Io2Board::ioMap() const {
    std::vector<MapEntry> m;
    m.push_back({(uint32_t)straps_.statusPort, (uint32_t)straps_.statusPort, "read/write",
                 "IO-2 -- status (R) / control, discarded (W)"});
    m.push_back({(uint32_t)straps_.dataPort, (uint32_t)straps_.dataPort, "read/write",
                 "IO-2 -- receive data (R) / transmit data (W)"});
    return m;
}

bool Io2Board::connect(const std::string& unit, const std::string& endpoint, std::string& err) {
    if (unit != "serial") {
        err = "io2 has no unit '" + unit + "' -- it has one, and it is called 'serial'";
        return false;
    }
    if (!g_resolver) {
...
bool Io2Board::disconnect(const std::string& unit, std::string& err) {
    if (unit != "serial") {
        err = "io2 has no unit '" + unit + "' -- it has one, and it is called 'serial'";
        return false;
    }
