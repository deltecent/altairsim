...
void Cpu8085Board::serialize(StateWriter& w) const {
    Board::serialize(w);
    core_->serialize(w);
}

void Cpu8085Board::deserialize(StateReader& r) {
    Board::deserialize(r);
    core_->deserialize(r);
}
...
std::vector<Property> Cpu8085Board::properties() {
...
void Cpu8085Board::publishPolicy() {
    if (!clock_) return;
    clock_->setHz(clockHz_);
    clock_->setIdle(idle_);
}

std::vector<UnitDef> Cpu8085Board::units() const {
    return {{"8085", UnitKind::Cpu, "active"}};
}
