...
`ddb200` and `msmonr21` are **SD Systems** parts. SD Systems is long defunct, and both circulate freely in the vintage archives (deramp.com): `ddb200` is Rex Brown's DDBIOS floppy driver and `msmonr21` is the SBC monitor PROM. They ship on the same footing as the MITS parts above — reversible to a path if either should not have been.
