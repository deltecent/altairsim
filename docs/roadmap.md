...
The board is `memory` (`docs/boards/s100-memory.md`): a card holding a list of **regions**, each `ram` (writes stored) or `rom` (writes **not decoded**), at 256-byte page granularity. Everything a memory card can be — a partly-populated RAM board, a PROM card of four sockets with two of them empty, a combo card with both — is that one list. Banking is not part of it: it is its own board, `bankmem` (`docs/boards/bankmem.md`).

**Two things make this milestone worth doing early, and neither needs a CPU.**

**Banking.** Four real cards bank in four genuinely different ways — Vector Graphic one-hot select-one, Cromemco 64KZ an 8-bit mask that can hold several banks live at once, North Star HRAM an on/off-plus-one-hot-toggle that isn't a bank number at all, and SD Systems ExpandoRAM II an approximated page-select — and Vector and Cromemco share a port with incompatible encodings. If the board API can express all four with no bus special case and no monitor change, the central claim of the whole design is proven **before a single CPU instruction is executed.**
...
3. `RESET` preserves memory; **only `POWER` loses it** — and `POWER` re-reads ROM images from their files.
4. `CONFIG SAVE` → `CONFIG LOAD` reproduces the regions and page map exactly.
...
9. **Banking, all four types** (now the `bankmem` board, `docs/boards/bankmem.md`, not `memory`). Vector's one-hot select; Cromemco 64KZ's 8-bit mask, several banks live at once; North Star's on/off-plus-one-hot-toggle; the ExpandoRAM II's approximated page-select; the Vector/OASIS `0x41`/`0x42` quirk.
10. **Two banked cards both claiming port 0x40** (Vector and Cromemco 64KZ do) → **I/O contention reported**, naming both.
...
- **`FilterStream`** (`src/host/filter.cpp`) — the transform chain, put **on the line rather than on the console** so `SET sio0:a UPPER=ON` would work identically on a socket. **That was reversed later, and the reversal is the rule now:** a transform is a *console* property (`SET CONSOLE UPPER=ON`), because a card's line has to stay **8-bit clean** — a filter sitting on it corrupts XMODEM, silently. What a card has instead is line *coding* (`baud`, `data_bits`), which is a frame, never a mask. `SET sio0:a UPPER=ON` is an error today, and says so.
...
|---|---|---|
| **1a — memory bench** | CLI, bus, `memory` board (ram + rom regions, PHANTOM\*; banking later split out to `bankmem`). **No CPU.** | Monitor acts as bus master: unpopulated pages read `FF`; a ROM region doesn't decode writes; all three phantom straps behave; contention detected; `RESET` keeps RAM, only `POWER` loses it |
...
| 2 — CPU gate | (none) | **DONE 2026-07-11** — TST8080 / 8080PRE / CPUTEST / **8080EXM** all pass (`ctest`, plus `ctest -L slow` for 8080EXM). **They run in CI too now**, on all three legs (Linux, a universal macOS binary, Windows/MSVC), on their own workflow rather than every push. The no-`#ifdef` lint is green. |
...
| 4 — memory model | ~~ROM board, PHANTOM, banking~~ (**built** — `docs/boards/s100-memory.md` + `docs/boards/bankmem.md`) | DBL PROM at 0xFF00 overlays RAM; `SHOW BUS MAP` shows it; bank switching works. **Done, since split in two:** `PhantomAssert`/`PhantomHonor` stayed on the `memory` board (`src/boards/s100-memory.h`), covered by `tests/test_roms.cpp` and `tests/test_phantom.cpp`. **Banking moved to its own board, `bankmem`** (`src/boards/bankmem.{h,cpp}`, `tests/test_bankmem.cpp`) — four real, manual-sourced decoders (Vector Graphic, Cromemco 64KZ, North Star HRAM, SD Systems ExpandoRAM II), no two alike; the unsourced "B810" card was dropped and the ExpandoRAM I (no I/O port) stayed plain `memory` |
| 5 — rest of serial | ~~88-SIO~~ (**built 2026-07-12, in 1b**), ~~88-ACR~~ (**built 2026-07-12** — `docs/boards/mits-88acr.md`; it *is* an 88-SIO B, plus an FSK modem the guest cannot observe), ~~88-LPC~~ (**built** — PR #135; the `lpc` board) | MITS BASIC from a cassette image; SIO's **inverted** status alongside 2SIO's true-sense |
...
| 5¾ — the parallel printer | ~~**88-C700**~~ (**built** — `docs/boards/mits-88c700.md`; the `c700` board) | Printed bytes reach an endpoint: `file:` captures them, `null` discards them. Not a planned milestone — it is recorded here because it was built and the table did not say so, which is the same gap the 88-MDS and the front panel rows above were filled to close. **The polled path only.** The real card also carries a single-level interrupt (per-character or per-CR/LF, SW2 #4) that is *not* modelled: the status byte reports the INTERRUPT ENABLE bit software wrote (`intEnabled_`), but no request is raised and no wire is pulled — a deliberately deferred addition ([#26](https://github.com/deltecent/altairsim/issues/26)). It is **not** the 88-LPC (the `lpc` board, **built** in PR #135) — a separate board milestone 5 also owed, since shipped |
| 6 — interrupt board & DMA | ~~88-VI, RTC~~ (**built 2026-07-13** — `docs/boards/mits-88virtc.md`; one card, `virtc`), ~~DMA~~ (**mechanism built 2026-07-16**) | VI priority across *several* boards; a DMA card steals cycles and the clock notices. **The VI half is done:** MITS Programming System II runs with interrupts enabled, its console vectored through VI7 → `RST 7`, and Ctrl-C breaks a runaway program back to the monitor (`acceptance-ps2-int`). **The DMA half is now done too — the *mechanism*.** `Board::requestsBus()`/`busMaster()` and the run loop's `serviceDma()` grant `pHLDA` at an instruction boundary and charge the stolen T-states to the clock (DESIGN §4.5). It is proved by a real memory-to-memory master in `tests/test_dma.cpp` — genuine RAM→RAM cycles, the 128-T-state theft read back out of `clock.now()`, cycle-steal interleaving with a running CPU, and a `BREAK MEM W` that catches the DMA's own write. No fictional shipping DMA board was invented (§0.1). **The first shipping bus master has since landed (2026-08): the Tarbell #2022's on-card Intel 8257** (`src/chips/i8257.{h,cpp}`, `docs/boards/tarbelldd.md`), which a `DMACNTL=TRUE` CP/M CBIOS drives to move sectors — `acceptance-tarbelldd-dma` boots such a disk to A> reading directories through it, and the theft shows in `clock.now()`. It was additive, the way the 88-VI was additive onto the `IntAck` cycle. |
| 7 — parallel, host bridge | 88-PIO, 88-4PIO, ~~**Host Bridge**~~ (**built 2026-07-13** — `docs/boards/hostbridge.md`; our own card, `hostbridge`, at 0xB0) | `R`/`W`/`HDIR` move files to and from the host; sandbox escape attempts are refused. **The Host Bridge is done.** It is the project's first genuinely new piece of hardware, and the board API needed nothing to take it. Its guest utilities are 8080 assembly **assembled inside the machine** by `ASM.COM`/`LOAD.COM` — `acceptance-hostbridge-build` `PIP`s the sources in through the console and builds them, so the `.ASM` and the card cannot drift, while `acceptance-hostbridge` runs the committed `.HEX` on the tracked floppy every time. It then round-trips 256 bytes containing every value from `00` to `FF` and compares them **on the host's disk**, and does the whole thing again with the *same binary* on an 88-MDS minidisk behind a different controller — which is how the "every disk call is a BDOS call" claim gets proved instead of asserted. The 88-PIO and 88-4PIO remain, and are still blocked on documentation (§17). |
| 8 — more cores | ~~8085~~ (**built 2026-08** — an `8085` CPU card, a minimal `8085` machine, and an `8085` disassembler/assembler), ~~Z80~~ (**built 2026-07-15** — `docs/boards/mits-z80cpu.md`; a `z80` CPU card and a minimal `z80` machine) | ZEXALL / ZEXDOC; a Z80 CP/M binary runs. **The Z80 half is done and validated:** ZEXDOC and ZEXALL both pass (`ctest` targets `cpu-zexdoc` / `cpu-zexall`, `-L slow`), and they passed before any board was built on the core — the same gate the 8080 went through. **The 8085 core is done and validated too:** RIM/SIM, TRAP + RST 5.5/6.5/7.5, and the documented set — including the 8085's faithful `ANA` auxiliary carry — pass against **real-silicon** CRCs (`cpu-8085exm`, `-L slow`), the same before-any-board gate. **The 8085's *faithful* reproduction is now done as well (issue #347, 2026-08):** all ten undocumented opcodes execute and the V/K flag bits compute, gated by hand-derived vectors whose oracle is external — Ken Shirriff's silicon-die analysis plus the Tundra CA80C85B data sheet and the 1979 *Electronics* article — because `8085EXM` masks V/K out and never emits the undocumented bytes. All that is left is wiring the 8085's `SID`/`SOD` and `TRAP`/`RST 5.5`/`6.5`/`7.5` *pins* to a board, deferred until a concrete 8085 board needs them (the S-100 bus carries no such line). |
| *later* | ~~PMMI MM-103~~ (**built** — `docs/boards/pmmi-mm103.md`; the `pmmi` board, `examples/pmmi/`), ~~VDM-1~~ (**built 2026-07-18** — `docs/boards/proctech-vdm1.md`, and the **Sol-20** with it), ~~Dazzler~~ (**built 2026-07-23** — `docs/boards/cromemco-dazzler.md`; `dazzler`, KSCOPE in `examples/dazzler/`) | see below |
...
Each milestone has a **written acceptance test that runs headless in CI on all three platforms** — Linux, macOS (one universal `x86_64`+`arm64` binary) and Windows — via `altairsim -s script.cmd`, so "it works" is never an opinion.
...
**The PMMI MM-103 has since been built, and it passed that test** — `docs/boards/pmmi-mm103.md`, the `pmmi` board, and the terminal in `examples/pmmi/`. Expressing it needed no new simulator plumbing: the read/write asymmetry at one port, the side-effect-only read, the write-only registers' shadow copies, and the register shared between the baud divisor and the dialer and the interrupt mask all fell out of the existing board API, exactly as the paper exercise predicted. The data path, the 6860 self-test loopback, the `ModemLine` status walk, and console passthrough are done. What remains deferred is the modem's **interrupts** and its **auto-dialer** — the semantics the 88-2SIO could not already stand in for, now that everything it *could* is in place.
...
**The VDM-1 has since been built (2026-07-18), and it passed that test** — `docs/boards/proctech-vdm1.md`, the `vdm1` machine, and an SDL3-backed `Display` seam that is an *optional* dependency (no SDL, no video boards, everything else still builds). The bus, the monitor and the MCP server were untouched. The **Sol-20** came with it (`docs/boards/proctech-sol.md`, the `sol20` machine): the integrated Sol-PC as one composite board, reusing that same VDM-1. **The Dazzler has since been built too (2026-07-23)** — `docs/boards/cromemco-dazzler.md`, the `dazzler` machine, and KSCOPE in `examples/dazzler/` — reusing that same `Display` seam and, again, touching neither the bus nor the monitor nor the MCP server. It renders from its framebuffer in main RAM the way the VDM-1 renders from its screen RAM; it deliberately does **not** bus-master (a correct picture only needs to read the current frame, which `Bus::peek()` does headlessly and deterministically). **The DMA mechanism has since carried its first shipping board (2026-08): the Tarbell #2022's on-card Intel 8257** (`docs/boards/tarbelldd.md`), which a `DMACNTL=TRUE` CP/M CBIOS drives to steal cycles for every sector read — retiring the "only a synthetic test" caveat.
...
4. ~~**A DMA card** — bus mastering, cycle stealing.~~ **Built 2026-08.** The Dazzler was the intended example but shipped rendering from RAM without mastering the bus; the **Tarbell #2022's on-card Intel 8257** became the first shipping bus master instead (`src/chips/i8257.{h,cpp}`, `docs/boards/tarbelldd.md`, `acceptance-tarbelldd-dma`).
