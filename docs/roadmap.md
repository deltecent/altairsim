...
| 8 — more cores | ~~8085~~ (**built 2026-08** — an `8085` CPU card, a minimal `8085` machine, and an `8085` disassembler/assembler), ~~Z80~~ (**built 2026-07-15** — `docs/boards/mits-z80cpu.md`; a `z80` CPU card and a minimal `z80` machine) | ZEXALL / ZEXDOC; a Z80 CP/M binary runs. **The Z80 half is done and validated:** ZEXDOC and ZEXALL both pass (`ctest` targets `cpu-zexdoc` / `cpu-zexall`, `-L slow`), and they passed before any board was built on the core — the same gate the 8080 went through. **The 8085 core is done and validated too:** RIM/SIM, TRAP + RST 5.5/6.5/7.5, and the documented set — including the 8085's faithful `ANA` auxiliary carry — pass against **real-silicon** CRCs (`cpu-8085exm`, `-L slow`), the same before-any-board gate. Remaining for the 8085 is *faithful* reproduction (issue #347): the ten undocumented opcodes and the V/K flag bits, each blocked on a genuine exerciser that covers them. |
