...
The **modem handshake and the software-controlled phone line are now built** (Phase 2). Two board properties — **`dial=host:port`** and **`answer=port`** — turn the one serial line into a `ModemLine` (a dynamic phone line over TCP, `src/host/modemline.{h,cpp}`). The guest's `OUT BA+0`/`OUT BA+3` writes drive it: **SH off-hook + DTR originates** the call to `dial=`, **DTR arms auto-answer** on `answer=` (an inbound connection *rings* until the guest answers), and **DTR-drop hangs up**. `IN BA+2` is no longer a constant — it is computed from the phone line plus a **Clock-driven handshake state machine** (ringing bursts, dial tone, the billing/CTS/AP timings below). See *How it is simulated* and `tests/test_pmmi.cpp`.
...
- Properties: `port`, `dial`, `answer`. `interrupt` and `carrier_delay` belong to later milestones. **Self-test is *not* a property** — it is guest-controlled by `OUT BA+3` bit 4, not an operator jumper, so it is surfaced read-only in the `lines` status string (capital `ST` when looped) rather than as a setter.
