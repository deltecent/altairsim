...
| File | What it is |
|---|---|
| `dualidesd.toml` | The machine: V2 Z80 CPU + MASTER EEPROM (`v2z80`), a `propio` console, the `dualide` board at `30h`–`34h` (drives A:/B:), the `dualsd` board at `80h`/`81h` (drives C:/D:), a front panel, the host bridge at `B0`/`B1`, and 64K RAM. |
