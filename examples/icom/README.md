...
Four ready-made machines for the **iCOM FD3712/FD3812** 8″ floppy controller — CP/M in single
and double density, and iCOM's own FDOS disk operating system. Each boots the moment you start
it (`RUN F000` for CP/M, `RUN C000` for FDOS is the machine file's whole startup, because on a
real machine that was EXAMINE the PROM address and RUN).
...
CP/M (it is warm boot) and CP/M gets it. FDOS's prompt is `!`; type `LIST` for a directory.
